...
interface SRTSectionProps {
	type: "publish" | "read";
	connections: SRTConnectionItem[];
	title: string;
}
...
export function SRTSection({ type, connections, title }: SRTSectionProps) {
		...
		<div className="group p-4 md:p-6 bg-white/70 backdrop-blur-xl rounded-xl md:rounded-2xl border border-gray-200/50 shadow-[0_8px_32px_0_rgba(31,38,135,0.1)] hover:shadow-[0_20px_40px_0_rgba(31,38,135,0.15)] transition-all duration-300 will-change-transform">
			<div className="flex items-center justify-between mb-4">
				<div className="flex items-center space-x-3">
					<div
						className={`w-10 h-10 bg-gradient-to-br ${config.gradient} rounded-lg flex items-center justify-center shadow-[0_8px_32px_0_rgba(31,38,135,0.15)] group-hover:shadow-[0_20px_40px_0_rgba(31,38,135,0.2)] transition-all duration-300`}
					>
						<svg
							className={`w-5 h-5 text-white ${config.iconAnimation} transition-transform duration-300 ease-out`}
							fill="currentColor"
							viewBox="0 0 24 24"
						>
					...
					<div>
						<h3
							className={`text-lg font-medium tracking-tight bg-gradient-to-r ${config.titleGradient} bg-clip-text text-transparent`}
						>
							{title}
						</h3>
						<div
							className={`w-8 h-[1px] bg-gradient-to-r ${config.gradient} rounded-full`}
						></div>
					</div>
				</div>
				<div className="flex items-center space-x-2 text-sm text-gray-600">
					<span
						className={`w-2 h-2 rounded-full ${statusDotColor} ${connections.length > 0 ? "animate-pulse" : ""}`}
					></span>
					<span className="font-light">
						{connections.length} active{connections.length > 1 ? "s" : ""}
					</span>
				</div>
			</div>
					...
					{connections.map((conn) => (
						...
						>
							<div className="flex items-center space-x-3 flex-1 min-w-0">
								<div
									className={`w-8 h-8 bg-gradient-to-br ${config.connectionGradient} rounded-lg flex items-center justify-center shadow-sm group-hover/item:shadow-md transition-all duration-200`}
								>
									<svg
								...
								<div className="flex-1 min-w-0">
									<div className="font-medium text-gray-900 text-sm truncate group-hover/item:text-sabs-primary transition-colors duration-200">
										{conn.path}
									</div>
								</div>
							</div>
							<span
								className={`px-2 py-1 rounded-md text-xs font-medium ${getStatusColor(conn.state)} transition-all duration-200`}
							>
