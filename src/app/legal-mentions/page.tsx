...
const ANCHOR_CLASS =
	"group relative text-sabs-primary hover:text-sabs-gradient-2 transition-colors duration-200 font-medium";

const PATTERN_STYLE = {
	backgroundImage:
		"radial-gradient(circle at 25% 25%, #3bd1ab 0%, transparent 70%), radial-gradient(circle at 75% 75%, #61437f 0%, transparent 70%)",
};
...
const MD_OVERRIDES = {
	h2: {
		component: ({ children, ...props }: React.PropsWithChildren) => (
			...
			>
				<div className="w-2 h-2 bg-linear-to-r from-sabs-gradient-1 to-sabs-gradient-2 rounded-full group-hover:scale-125 transition-transform duration-300"></div>
				{children}
			</h2>
		),
	},
		...
		component: ({
			...
			return (
				<a
					...
					aria-label={isExternal ? `Lien externe vers ${children}` : undefined}
				>
					{children}
					<span className="absolute bottom-0 left-0 w-0 h-0.5 bg-linear-to-r from-sabs-gradient-1 to-sabs-primary group-hover:w-full transition-all duration-300"></span>
					{isExternal && (
						<svg
...
export default function LegalMentions() {
	...
	return (
		<div
			ref={containerRef}
			className="min-h-screen relative bg-linear-to-br from-slate-50 via-gray-100 to-slate-200 py-12"
			style={{ fontFamily: beVietnam.style.fontFamily }}
		>
			<div className="absolute inset-0 opacity-[0.02]">
				<div className="absolute inset-0" style={PATTERN_STYLE}></div>
			</div>

			<div className="relative z-10 px-6 max-w-4xl mx-auto">
				<div
					ref={titleRef}
					className="text-center mb-8 opacity-0 translate-y-12"
				>
					<h1 className="text-3xl md:text-4xl lg:text-5xl font-thin text-gray-900 mb-4 tracking-[-0.02em]">
						Mentions{" "}
						<span className="text-sabs-primary font-medium">Légales</span>
					</h1>
					<div className="w-16 md:w-24 h-0.5 bg-linear-to-r from-sabs-gradient-1 via-sabs-gradient-2 to-sabs-primary mx-auto rounded-full mb-4"></div>
					<p className="text-gray-700 text-lg font-light">
						Informations légales (HORS RÔLEPLAY)
					</p>
				</div>

				<div
					ref={contentRef}
					className="bg-white/70 backdrop-blur-xl rounded-2xl md:rounded-3xl p-6 md:p-8 border border-gray-200/50 shadow-[0_8px_32px_0_rgba(31,38,135,0.1)] hover:shadow-[0_20px_40px_0_rgba(31,38,135,0.15)] transition-all duration-500 opacity-0 translate-y-12"
				>
					<div className="markdownApplicable">
						<Markdown options={{ overrides: MD_OVERRIDES }}>
							{frenchMd}
						</Markdown>
					</div>

					<div className="mt-8 pt-6 border-t border-gray-200/50">
						<div className="bg-linear-to-r from-sabs-primary/10 to-sabs-gradient-2/10 backdrop-blur-sm rounded-2xl p-6 border border-sabs-primary/20 shadow-lg">
							...
							<p className="text-gray-700 leading-relaxed">
								Pour toute question concernant ces mentions légales ou ce site
								web, vous pouvez me contacter à l&apos;adresse{" "}
								<a
